...
class Candb_msg_field:
    ...
    def __str__(self):
        ...
        for idx in range(self.count):
            if self.field_type in [Candb_msg_field_type.INT, Candb_msg_field_type.UINT, Candb_msg_field_type.BOOL]:
                ...
                v += "{} (={:2d})".format(
                    self.linked_enum.enum[self.value[idx]].name, self.linked_enum.enum[self.value[idx]].value
                )
        ...
        v = indent + "{:5} ({:2}:{:2} = {:2}) [{:1}] = {} {}".format(
            self.field_type.name,
            self.pos_offset + self.bits,
            self.pos_offset,
            self.bits,
            self.count,
            v,
    ...
    def parseFromPacket(self, dataList):
        for arr in range(self.count):
            if self.field_type in [Candb_msg_field_type.UINT, Candb_msg_field_type.BOOL, Candb_msg_field_type.MUX]:
                ...
                raw = self.linked_enum.enum[
                    extractUnsignedInt(dataList, self.pos_offset + arr * self.bits, self.bits)
                ].value  # translate number to enum element (value, name)
            else:
    ...
    def assemble(self, values, buff):
        if self.field_type == Candb_msg_field_type.MUX:
            ...
            Candb_msg_field_type.UINT,
            Candb_msg_field_type.BOOL,
            Candb_msg_field_type.MUX,
            Candb_msg_field_type.INT,
            Candb_msg_field_type.ENUM,
        ]:
                ...
                for unit in pkg.get("units"):  # iterate over units (owners of messages)
                    ...
                    for j_enum in unit.get("enum_types"):  # parse enums
                        ...
                        for item in j_enum.get("items"):
                            enum.append(
                                Custom_enum_element(
                                    "{}".format(item.get("name")), item.get("value"), item.get("description")
                                )
                            )
                    ...
                    for msg in unit.get("messages"):
                        ...
                        m = Candb_msg(
                            description=Description(msg.get("name"), text=msg.get("description")),
                            owner=msg_owner,
                            ...
                            busname=bus.split(".")[-1],
                        )  # full bus name includes package name, ignore it
                        ...
                        for field in msg.get("fields"):
                            ...
                            if field.get("type") == "reserved":
                                ...
                                if len(name) > 0:
                                    ...
                                    m.add_field(f)
                        self.parsed_messages[m.busname == "CAN3"][m.identifier] = m

    def isMsgKnown(self, busId, msgId):
        return msgId in self.parsed_messages[busId]

    def getMsgById(self, busId, msgId) -> Candb_msg:
        return self.parsed_messages[busId][msgId]

    def getMsgByNameAndUnit(self, msgName: str, unitName: str):
        for i in range(len(self.parsed_messages)):
            pass

    def parseData(self, busId, msgId, dataList, timestamp):
        if not self.isMsgKnown(busId, msgId):
        ...
        msg = self.getMsgById(busId, msgId)
